...
public class ExtentTestNGListener implements ITestListener {
    ...
    public void onTestSuccess(ITestResult result) {
        test.get().log(Status.PASS, "Test Başarılı geçti");
    }

    @Override
    public void onTestFailure(ITestResult result) {
        test.get().log(Status.FAIL, "Test Başarısız: " + result.getThrowable().getMessage());
        // İstersen buraya screenshot ekleme kodu da yazabilirsin
    }

    @Override
    public void onTestSkipped(ITestResult result) {
        test.get().log(Status.SKIP, "Test Atlandı: " + result.getThrowable().getMessage());
    }
